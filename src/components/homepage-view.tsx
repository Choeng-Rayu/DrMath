...
export function HomepageView({ content, hiddenKeys, settings, subjects, testimonials, videos }: HomepageViewProps) {
  const hidden = (key: string) => (hiddenKeys.includes(key) ? { "data-cms-hidden": "true" } : {});
  ...
  return (
    <main className="site-shell">
      <div className="topbar" {...hidden("nav.cta")}>
      ...
      <section id="home" className="hero paper-grid">
        <div className="container hero-grid">
          <div>
            ...
            <h1 className="display" data-cms-key="hero.title" data-cms-rich="true">{plain(content["hero.title"])}</h1>
            <p className="hero-copy" data-cms-key="hero.description" data-cms-rich="true">{content["hero.description"]}</p>
            <div className="hero-actions">
              <a className="button button-primary" href={content["hero.primaryUrl"]} {...external(content["hero.primaryUrl"])}><span data-cms-key="hero.primaryLabel">{content["hero.primaryLabel"]}</span><Send size={17} aria-hidden="true" /></a>
              <a className="button button-secondary" href={content["hero.secondaryUrl"]} {...hidden("hero.secondaryLabel")}><BookOpen size={17} aria-hidden="true" /><span data-cms-key="hero.secondaryLabel">{content["hero.secondaryLabel"]}</span></a>
            </div>
          ...
          <div className="hero-art" aria-label="ផ្ទាំងរូបភាព DR.MATHS">
            <div className="hero-poster" {...hidden("hero.imageDriveUrl")}>
              <img src={heroImage?.renderUrl ?? "/images/Photo-A.jpg"} alt={heroImage ? "រូបភាពសកម្មភាពសិក្សា DR.MATHS" : "ផ្ទាំងរូបភាព DR.MATHS"} data-cms-key="hero.imageDriveUrl" data-drive-original={content["hero.imageDriveUrl"] || ""} referrerPolicy="no-referrer" />
      ...
      <section className="stats" aria-label="ស្ថិតិ DR.MATHS">
        <div className="container stats-grid">
          {[1, 2, 3, 4].map((number) => (
            <div className="stat" key={number} {...hidden(`stats.${number}.value`) && hidden(`stats.${number}.label`)}>
              <div className="stat-value" data-cms-key={`stats.${number}.value`}>{content[`stats.${number}.value`]}</div>
              <div className="stat-label" data-cms-key={`stats.${number}.label`}>{content[`stats.${number}.label`]}</div>
            </div>
          ))}
        </div>
      </section>
        ...
        <div className="container">
          ...
          <div className="about-grid" style={{ marginTop: "2rem" }}>
            <div style={{ display: "grid", gap: "1rem" }}>
              <article className="info-card" {...hidden("about.vision")}><h3 data-cms-key="about.visionTitle">{content["about.visionTitle"]}</h3><p data-cms-key="about.vision" data-cms-rich="true">{content["about.vision"]}</p></article>
              <article className="info-card" {...hidden("about.mission")}><h3 data-cms-key="about.missionTitle">{content["about.missionTitle"]}</h3><p data-cms-key="about.mission" data-cms-rich="true">{content["about.mission"]}</p></article>
            </div>
            <article className="about-long" {...hidden("about.long")}>
              <Sparkles color="#FFB703" size={25} aria-hidden="true" />
              <img className="about-photo" src={aboutImage?.renderUrl ?? "/images/A-2025_only.jpg"} alt={aboutImage ? "រូបភាពសកម្មភាពសិក្សា DR.MATHS" : "រូបភាព DR.MATHS"} loading="lazy" referrerPolicy="no-referrer" data-cms-key="about.imageDriveUrl" data-drive-original={content["about.imageDriveUrl"] || ""} />
      ...
      <section className="section">
        <div className="container">
          ...
          <div className="format-grid">
            {formats.map((format) => (
              <article className="format-card" key={format.number} {...hidden(`formats.${parseInt(format.number, 10)}.title`) && hidden(`formats.${parseInt(format.number, 10)}.text`)}>
                <div className="format-number">{format.number}</div>
                <h3 data-cms-key={`formats.${parseInt(format.number, 10)}.title`}>{format.title}</h3>
                <p data-cms-key={`formats.${parseInt(format.number, 10)}.text`} data-cms-rich="true">{format.text}</p>
              </article>
            ))}
          </div>
        </div>
        ...
        <div className="container">
          ...
          <div className="highlight-grid">
            {highlights.map((highlight, index) => (
              <article className="highlight-card" key={highlight.title} {...hidden(`highlights.${index + 1}.title`) && hidden(`highlights.${index + 1}.text`)}>
                <span className="format-number">{`0${index + 1}`}</span>
                <h3 data-cms-key={`highlights.${index + 1}.title`}>{highlight.title}</h3>
                <p data-cms-key={`highlights.${index + 1}.text`} data-cms-rich="true">{highlight.text}</p>
              </article>
            ))}
          </div>
        </div>
      ...
      <section id="contact" className="section contact">
        <div className="container contact-grid">
          <div>
            ...
            <a className="button button-primary" href={settings.telegramUrl ?? content["hero.primaryUrl"]} {...external(settings.telegramUrl ?? content["hero.primaryUrl"])}><span data-cms-key="contact.cta">{content["contact.cta"]}</span><Send size={17} aria-hidden="true" /></a>
          </div>
